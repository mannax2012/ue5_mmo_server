...
void Entity::MoveTo(float newX, float newY, float newZ) {
    ...
    LOG_DEBUG("Entity::MoveTo coordinate delta: " + std::to_string(id) + " distance=" + std::to_string(distance) + " delta=(" + std::to_string(newX - x) + ", " + std::to_string(newY - y) + ", " + std::to_string(newZ - z) + ")");
    float oldX = x, oldY = y;
   ...
   if (zoneManager) {
        ...
        LOG_DEBUG("Entity::MoveTo nearbyZones: " + 
